...
public class GoogleIntegration {
    ...
    private static final String DATEFORMAT_GOOGLE_SYNC = "YYYY-MM-dd";
    ...
    private static final String FILEDATASTORE_PARENT_DIRECTORY = System.getProperty("user.home");
    ...
    private void init() throws GeneralSecurityException, IOException {
        ...
        if (_shouldRememberLogin) {
            File f = new File(FILEDATASTORE_PARENT_DIRECTORY, FILEDATASTORE_CHILD_DIRECTORY);
            try {
    ...
    public void sync(List<Task> taskList, List<String> deletedIds) {
        ...
        try {
            ...
            syncWithGoogleCalendar(taskList, deletedIds, _googleCalendar, _googleTasks,
                                   calId, taskListId);
            
            LOGGER.info(LOG_GOOGLE_TASKS_SYNC);
            syncWithGoogleTask(taskList, deletedIds, _googleCalendar, _googleTasks, calId,
                               taskListId);
    ...
    private void syncWithGoogleTask(List<Task> taskList, List<String> deletedIds,
    ...
    private void syncWithGoogleCalendar(List<Task> taskList, List<String> deletedIds,
            GoogleCalConn googleCalendar, GoogleTaskConn googleTasks,
    ...
    private void updateTaskWithGoogleTask(Task t,
        ...
        if (gt.getDue() != null) {
            ...
            cal.add(java.util.Calendar.DATE, OFFSET_DUEDATE);
            cal.add(java.util.Calendar.SECOND, OFFSET_DUESECOND);
            t.setDueDate(cal);
            t.setHasDueTime(false);
        }
    ...
    private boolean isEmptyGoogleTask(com.google.api.services.tasks.model.Task t) {
        ...
        // 12.14
        if ((t.getTitle().length() == LENGTH_NEXTLINECHAR) && (t.getTitle().equals(STRING_NEXTLINECHAR))) {
            return true;
        }
        return false;
    }
    ...
    private DateTime calendarToDate(java.util.Calendar c) {
        SimpleDateFormat sdf = new SimpleDateFormat(DATEFORMAT_GOOGLE_SYNC);
